  ...
  return (
    <Dialog open={isComplete} onOpenChange={setIsComplete}>
        ...
        <DialogHeader>
          ...
          <div className="flex justify-center gap-2 mb-4">
            {showStars.map((show, index) => (
              <div
                ...
                style={{ 
                  animation: show ? `bounce 0.6s ease-out ${index * 0.1}s 1 forwards` : 'none'
                }}
              >
                ⭐
              </div>
          ...
          <DialogTitle className={`transition-all duration-500 ${showButtons ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            🎉 Lesson Complete!
          </DialogTitle>
          <DialogDescription className={`transition-all duration-500 delay-100 ${showButtons ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            Congratulations! You've completed all the lesson objectives. What would you like to do next?
          </DialogDescription>
        </DialogHeader>
        
        <div className={`flex flex-col gap-3 mt-4 transition-all duration-500 delay-200 ${showButtons ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <Link href="/dashboard">
          ...
          <Link href={`/learn/${lessonIndex}/quiz`}>
            <Button variant="outline" className="w-full transform transition-transform hover:scale-105">
              Take the Quiz
            </Button>
          </Link>
          <Button 
